...
struct ListElem {
    ...
    struct ListElem* prev;  // Pointeur vers l'élément précédent
};
...
LinkedList* linked_list_new() {
    ...
    l->sentinel_end.prev = &(l->sentinel_start);  // Sentinelle de fin pointe vers la sentinelle de début
    l->sentinel_end.next = &(l->sentinel_end);    // La sentinelle de fin pointe vers elle-même pour éviter les erreurs
    l->sentinel_start.prev = &(l->sentinel_start); // La sentinelle de début pointe vers elle-même
    return l;
}
...
// Fonction pour insérer un élément avant un élément donné
void linked_list_insert_before(ListElem* elem, float toInsert) {
    ListElem* newElem = (ListElem*)malloc(sizeof(ListElem));
    newElem->value = toInsert;
